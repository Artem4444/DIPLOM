...
class AuthorizationState extends State {
  ...
  _sendUserLogin(User user) async {
    ...
    if (!NetworkManager.isConected)
      ...
      if (response) {
        user.id ="1";
        user.firstName="Артём";
        user.secondName="Супрунович";
        user.password="1111";
        user.mobileNumber="+375298034287";
        String jsonUser = jsonEncode(user);
        await AppRepository.setLocalUser(jsonUser);
        _showGreatings();
      } else
        _showWarning("Данные введены не корректно");
    }
  }
  ...
  void _registrateUser(User user) async {
    ...
    if (!NetworkManager.isConected)
      _showNoConection();
        ...
        if (response) {
          _showSucces("Пользователь ${user.secondName} ${user.firstName} успешно зарегистрирован!");
          authorizationPage = AuthorizationPage.login;
        } else {
          _showWarning("Данный пользователь уже зарегистрирован");
        }
    }
  }
